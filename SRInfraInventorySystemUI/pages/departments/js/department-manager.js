...
export default class DepartmentManager {
        ...
        try {
            ...
            if (window.select2Instances) {
                if (window.select2Instances['parent-department-wrapper']) {
                    const opts = this.buildOptionsArray(departments, 'Üst Departman Yok');
                    window.select2Instances['parent-department-wrapper'].updateOptions(opts);
                }
                if (window.select2Instances['filter-parent-wrapper']) {
                    const opts = this.buildOptionsArray(departments, 'Tümü');
                    window.select2Instances['filter-parent-wrapper'].updateOptions(opts);
                }
            }
        } catch (error) {
    ...
    // Select2Style icin dizi formatinda option olustur
    buildOptionsArray(departments, firstText = '') {
        const opts = [];
        if (firstText !== null) {
            opts.push({ value: '', text: firstText });
        }
        const traverse = (list) => {
            list.forEach(d => {
                if (!d) return;
                ...
                opts.push({ value: id.toString(), text: name });
                const children = d.subDepartments || d.children || [];
                if (children.length > 0) {
                    traverse(children);
                }
            });
